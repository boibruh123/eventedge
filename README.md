...
Important: keep browser files such as `app.js`, `styles.css`, and `index.html` inside the `public/` folder. If `app.js` is placed at the project root, Vercel may try to run it as server code and show `ReferenceError: document is not defined`.
